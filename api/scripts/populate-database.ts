...
async function populateDatabase() {
  ...
  try {
    ...
    for (const [brandName, brandData] of Object.entries(carData)) {
      ...
      for (const [modelName, trims] of Object.entries(brandData.models)) {
        ...
        for (const trimName of trims) {
          try {
            await client.query(
              'INSERT INTO Car (Brand, Model, Trim) VALUES ($1, $2, $3)',
    ...
    brandSummary.rows.forEach((row) => {
      console.log(`   ${row.brand}: ${row.car_count} cars`);
    });
